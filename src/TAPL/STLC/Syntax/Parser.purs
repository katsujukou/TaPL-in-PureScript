module TAPL.STLC.Syntax.Parser where
...
import TAPL.STLC.Syntax.Lexer (LexerState, runLexer, tokenize)
import TAPL.STLC.Syntax.Position (SourcePhrase, mapPhrase, (..), (@@), (~))
import TAPL.STLC.Syntax.Types (Ann, Binder(..), Const(..), Expr(..), Keyword(..), SourceToken, Token(..), Type_(..), exprAnn, typeAnn)
...
parseBinder = binder <|> parensBinder 
  where
  binder = defer \_ -> do
    name <- ident
    typ <- optional (colon *> parseType)
    pure $ Binder { pos: name.at } name.it typ

  parensBinder = defer \_ -> do
    _ <- leftParens
    bin <- binder 
    _ <- rightParens 
    pure bin
